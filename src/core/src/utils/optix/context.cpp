    ...
    Context::build_accel(const OptixBuildInput &build_input)
{
    ...
    CUDABuffer as_temp_buffer(uncompacted_sizes.tempSizeInBytes);
    CUDABuffer as_buffer(compacted_size_offset + sizeof(size_t));
    ...
    {
        CUDABuffer compacted_as_buffer(compacted_size);
