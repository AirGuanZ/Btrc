...
void Scene::preprocess(optix::Context &optix_ctx)
{
    ...
    auto light_sampler = newRC<UniformLightSampler>();
    ...
    for(auto &inst : instances_)
    {
        ...
        if(inst.light)
        {
            light_sampler->add_light(inst.light);
    ...
    if(env_light_)
        light_sampler->add_light(env_light_);
    ...
    instance_info_        = CUDABuffer<InstanceInfo>(instance_info);
    geometry_info_        = CUDABuffer<GeometryInfo>(geometry_info);
    instance_to_material_ = CUDABuffer<int32_t>(instance_to_material);
}
