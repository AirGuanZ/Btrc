...
{
    return (path_state & PATH_FLAG_ACTIVE) != 0;
}

cuj::boolean is_path_intersected(cuj::u32 path_state)
{
    return (path_state & PATH_FLAG_HAS_INTERSECTION) != 0;
}

cuj::boolean is_path_scattered(cuj::u32 path_state)
{
    return (path_state & PATH_FLAG_HAS_SCATTERING) != 0;
}

cuj::u32 extract_instance_id(cuj::u32 path_state)
{
    return path_state & PATH_FLAG_INSTANCE_ID_MASK;
}
